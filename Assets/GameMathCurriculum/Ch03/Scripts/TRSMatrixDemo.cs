...
public class TRSMatrixDemo : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        Quaternion rotQ = Quaternion.Euler(0, rotationAngle, 0f);

        currentTRS = Matrix4x4.TRS(translation, rotQ, scale);

        transformedPoint = currentTRS.MultiplyPoint3x4(baseShape[1]);
    ...
    private void OnDrawGizmos()
    {
        ...
        if (showSteps)
        {
            ...
            Matrix4x4 rts = r * t * s;
            Color wrongC = new Color(0.5f , 0.5f , 0.5f);

            DrawShape(originPos,rts, wrongC,"잘못된 순서(RTS)");

            Vector3 wr=originPos+rts.GetPosition();
