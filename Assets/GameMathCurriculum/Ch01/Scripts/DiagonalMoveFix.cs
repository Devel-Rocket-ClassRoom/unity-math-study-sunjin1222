...
public class DiagonalMoveFix : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        // TODO
        Vector3 moveDirectionl = new Vector3(h, 0f, v);
        if (useNormalized)
        {
            moveDirectionl.Normalize();
        }
        transform.position = moveDirectionl * moveSpeed*Time.deltaTime;

        currentInputDirection = moveDirectionl;
        currentInputMagnitude = moveDirectionl.magnitude;
        currentSpeed = moveDirectionl.magnitude * moveSpeed;
        UpdateUI();
    }
